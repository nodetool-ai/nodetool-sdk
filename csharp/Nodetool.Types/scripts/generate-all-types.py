...
def generate_types_for_source(source_name: str, classes: List[type[BaseType]], output_dir: str, base_namespace: str) -> tuple[int, int]:
    ...
    if source_name == "core":
        ...
        source_dir = os.path.join(output_dir, "Types", "Core")
    else:
    ...
    for cls in classes:
        try:
            ...
            print(f"[ERROR] Error generating {cls.__name__} from {source_name}: {e}")
            errors += 1
...
def generate_nodes_for_source(source_name: str, nodes: List[type[BaseNode]], output_dir: str, base_namespace: str) -> tuple[int, int]:
    """Generate C# classes for nodes from a specific source."""
    generated = 0
    errors = 0
    
    # Create namespace-specific directory
    namespace = f"{base_namespace}.Nodes.{source_name.capitalize()}"
...
def get_package_name(raw_name: str) -> str:
    ...
    if raw_name.startswith("nodetool-"):
        name = raw_name[9:]
    else:
        name = raw_name
...
def generate_package_summary(output_dir: str, package_name: str, types: list, nodes: list, namespace: str) -> str:
    """Generate a summary file for a specific package. Returns the generated file name."""
    # Convert package name to proper C# format
    cs_package_name = get_package_name(package_name)
    summaries = []
    
    # Generate Types summary
    if types:
        types_summary_path = os.path.join(output_dir, "Types", f"{cs_package_name}.cs")
...
/// <summary>
/// Type information for {cs_package_name} package.
/// </summary>
public static class {cs_package_name}
{{
    #region Package Types
{chr(10).join(f'    public static readonly System.Type {class_name} = typeof({full_namespace}.{class_name});' for full_namespace, class_name in types)}
    #endregion
}}
""")
        summaries.append(f"{cs_package_name}.cs")
    
    # Generate Nodes summary
    if nodes:
        nodes_summary_path = os.path.join(output_dir, "Nodes", f"{cs_package_name}.cs")
...
/// <summary>
/// Node information for {cs_package_name} package.
/// </summary>
public static class {cs_package_name}
{{
    #region Package Nodes
{chr(10).join(f'    public static readonly System.Type {class_name} = typeof({full_namespace}.{class_name});' for full_namespace, class_name in nodes)}
    #endregion
}}
""")
        summaries.append(f"{cs_package_name}.cs")
...
def generate_summary_file(output_dir: str, namespace: str) -> None:
    """Generate summary files for all types and nodes."""
    print("\n>>> Generating summary files...")
    
    # Collect all .cs files recursively and organize by category
    core_types = []  # Core types
    package_types = {}  # Package-specific types
    nodes = {}  # All nodes by package
    
    for root, _, files in os.walk(output_dir):
        for file in files:
            if file.endswith(".cs") and not file.endswith("Types.cs") and not file.endswith("Nodes.cs"):  # Skip summary files
                rel_path = os.path.relpath(os.path.join(root, file), output_dir)
                namespace_parts = os.path.dirname(rel_path).split(os.sep)
                class_name = os.path.splitext(os.path.basename(file))[0]
                
                # Safely check path parts
                if len(namespace_parts) >= 2:  # Need at least Types/Core or Nodes/Core
                    if namespace_parts[0] == "Types":
                        package_name = namespace_parts[1]
                        if package_name == "Core":
                            # Core types
                            core_types.append((namespace, class_name))
                        else:
                            # Package-specific types
                            full_namespace = f"{namespace}.{package_name}"
                            if package_name not in package_types:
                                package_types[package_name] = []
                            package_types[package_name].append((full_namespace, class_name))
                    elif namespace_parts[0] == "Nodes":
                        # Nodes
                        package_name = namespace_parts[1]
                        full_namespace = f"{namespace}.Nodes.{package_name}"
                        if package_name not in nodes:
                            nodes[package_name] = []
                        nodes[package_name].append((full_namespace, class_name))
    
    # Sort all lists
    core_types.sort()
    for package in package_types:
        package_types[package].sort()
    for package in nodes:
        nodes[package].sort()
    
    # Generate core summaries
    if core_types:
        # Generate core types summary
        core_dir = os.path.join(output_dir, "Types")
        os.makedirs(core_dir, exist_ok=True)
        core_summary = os.path.join(core_dir, "CoreTypes.cs")
        with open(core_summary, "w", encoding="utf-8") as f:
            f.write(f"""//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by the NodeTool SDK Type Generator.
//     Runtime Version: Python
...
{chr(10).join(f'        public static readonly System.Type {class_name} = typeof({full_namespace}.{class_name});' for full_namespace, class_name in core_types)}
        #endregion
    }}
}}
""")
        print(f"Generated core summary: CoreTypes.cs")

    # Generate core nodes summary if needed
    if "Core" in nodes:
        core_nodes_dir = os.path.join(output_dir, "Nodes")
        os.makedirs(core_nodes_dir, exist_ok=True)
        core_nodes_summary = os.path.join(core_nodes_dir, "CoreNodes.cs")
        with open(core_nodes_summary, "w", encoding="utf-8") as f:
            f.write(f"""//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by the NodeTool SDK Type Generator.
//     Runtime Version: Python
...
namespace {namespace}.Nodes.Core;
...
{chr(10).join(f'        public static readonly System.Type {class_name} = typeof({full_namespace}.{class_name});' for full_namespace, class_name in nodes["Core"])}
        #endregion
    }}
}}
""")
        print(f"Generated core nodes summary: CoreNodes.cs")
    
    # Generate package-specific summaries
    generated_summaries = []
    for package_name in set(list(package_types.keys()) + list(nodes.keys())):
        if package_name != "Core":  # Skip Core as it's already handled
            pkg_types = package_types.get(package_name, [])
            pkg_nodes = nodes.get(package_name, [])
            if pkg_types or pkg_nodes:
                summary_file = generate_package_summary(output_dir, package_name, pkg_types, pkg_nodes, namespace)
                generated_summaries.append((package_name, summary_file))
    
    # Generate main summary file
    summary_path = os.path.join(output_dir, "NodeToolTypes.cs")
    with open(summary_path, "w", encoding="utf-8") as f:
        # Convert package names for documentation
        doc_packages = [(pkg_name, get_package_name(pkg_name)) for pkg_name, _ in generated_summaries]
...
{chr(10).join(f'/// - {cs_name}: Nodetool.Sdk.Types.{cs_name}' for _, cs_name in doc_packages)}
/// 
/// Available Nodes:
/// - Core: Nodetool.Sdk.Nodes.Core
{chr(10).join(f'/// - {cs_name}: Nodetool.Sdk.Nodes.{cs_name}' for _, cs_name in doc_packages)}
/// 
/// Usage:
...
{{
    ...
    {{
        #region Core Types
{chr(10).join(f'        public static readonly System.Type {class_name} = Core.{class_name};' for _, class_name in core_types)}
        #endregion

{chr(10).join(f'''        #region {cs_name} Types
{chr(10).join(f'        public static readonly System.Type {class_name} = {cs_name}.{class_name};' for full_namespace, class_name in pkg_types)}
        #endregion
''' for pkg_name, (cs_name, pkg_types) in [(p, (get_package_name(p), t)) for p, t in package_types.items()])}
    }}
    ...
    {{
        #region Core Nodes
{chr(10).join(f'        public static readonly System.Type {class_name} = Core.{class_name};' for _, class_name in nodes.get("Core", []))}
        #endregion

{chr(10).join(f'''        #region {cs_name} Nodes
{chr(10).join(f'        public static readonly System.Type {class_name} = {cs_name}.{class_name};' for full_namespace, class_name in pkg_nodes)}
        #endregion
''' for pkg_name, (cs_name, pkg_nodes) in [(p, (get_package_name(p), n)) for p, n in nodes.items() if p != "Core"])}
    }}
}}
""")
    
    print(f"Generated main summary file: NodeToolTypes.cs")
    print("Generated package summaries:")
    print(f"  - Core: CoreTypes.cs")
    for pkg_name, summary_file in generated_summaries:
        print(f"  - {get_package_name(pkg_name)}: {summary_file}")
    print(f"\nTotal types organized:")
    print(f"  - Core types: {len(core_types)}")
    print(f"  - Package types: {sum(len(t) for t in package_types.values())}")
    print(f"  - Nodes: {sum(len(n) for n in nodes.values())}")
    print(f"  - Packages with summaries: {len(generated_summaries) + 1}")  # +1 for core
