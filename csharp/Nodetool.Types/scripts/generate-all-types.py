...
def generate_summary_file(output_dir: str, namespace: str) -> None:
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        doc_packages = [(name, get_package_name(name)) for name in set(list(package_types.keys()) + list(nodes.keys()))]
        doc_packages.sort(key=lambda x: x[1])  # Sort by C# name
...
{chr(10).join(f'/// - {name}' for name in core_types)}
/// 
/// Package Types:
{chr(10).join(f'/// {cs_name} (Nodetool.Types.{cs_name}):' + chr(10) + chr(10).join(f'/// - {name}' for name in package_types.get(pkg_name, [])) for pkg_name, cs_name in doc_packages if pkg_name in package_types)}
/// 
/// Core Nodes (Nodetool.Nodes.Core):
{chr(10).join(f'/// - {name}' for name in nodes.get('Core', []))}
/// 
/// Package Nodes:
{chr(10).join(f'/// {cs_name} (Nodetool.Nodes.{cs_name}):' + chr(10) + chr(10).join(f'/// - {name}' for name in nodes.get(pkg_name, [])) for pkg_name, cs_name in doc_packages if pkg_name in nodes)}
/// 
/// Usage:
/// 
/// 1. Types:
///    using Nodetool.Types.Core;
///    using Nodetool.Types.Huggingface;
///    
///    var audio = new AudioRef();
///    var classifier = new AudioClassifier();
/// 
/// 2. Nodes:
///    using Nodetool.Nodes.Core;
///    using Nodetool.Nodes.Huggingface;
///    
///    var node = new AudioClassifier();
/// 
...
""")
    ...
    print(f"  - Packages with types/nodes: {len(doc_packages)}")
